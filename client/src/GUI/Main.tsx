...
export default function Main({ setWB, setDiff, diff }: { setWB: (x: boolean) => void, setDiff: (x: number) => void, diff: number }) {
  ...
  return (
    <div className="w-full h-full flex flex-col justify-center items-center relative">
      {
        warning ?
          <div className='text-white absolute under-390:top-16 top-24 left-1/2 transform -translate-x-1/2 above-450:w-1/3 under-450:w-1/2 bg-red-300/65 p-2 rounded-lg'>
            <h1 className='text-lg text-center onefont text-stroke under-390:text-xxs under-450:text-sm under-519:text-base above-460:text-pretty break-keep'>
              주의: 화면 가로 크기가 너무 작을 경우 게임이 올바르게 동작하지 않을 수 있습니다.
            </h1>
            <p className='text-xs under-450:text-xxs text-center mt-3 onefont s-text-stroke text-pretty'>화면 크기에 따른 버그 제보의 경우, gccs457@naver.com으로 제보해주세요.</p>
          </div> : null
      }
      ...
      <div className="absolute above-1024:right-5 under-1023:bottom-6">
      ...
      <h1 className="text-center max-sm:text-xl sm:text-4xl under-390:text-base text-white w-1/5 my-10 px-3 py-3 border-2 border-gray-300/35 rounded-xl newfont"> OMOK </h1>
